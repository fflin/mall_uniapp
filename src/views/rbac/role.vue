	...
	<div>
		<com-card label="角色列表">
			<template slot="content">
				...
				<!-- 抽屉弹框 -->
				<el-drawer :visible.sync="drawer" :with-header="false" @close="closeKeys">
					<div class="header">
						<span>配置权限</span>
...
export default {
	...
	methods:{
		...
		async drawers(row){//授权
			...
			this.upLoadForm.rules = row.rules
			this._id = row._id
			//递归获取权限节点
			this.getrulekeys(row,this.defKeys)
			this.drawer = true
		},
		// 通过递归获取角色下所有权限ID
		getrulekeys(doc,arr){
			if(!doc.children){
				return arr.push(doc._id)
			}
			//使用递归(保存到arr)
			doc.children.forEach(item => {
				this.getrulekeys(item,arr)
			})
		},
		...
		async submit(){//权限提交
			...
			const res = await this.axios.put(`/admin/roles/power/${this._id}`,{rules: idStr})
			this.drawer = false,
			this.init()
