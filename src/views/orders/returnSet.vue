<template>
	<com-card label="退货原因">
		<template slot="content">
			<!-- 新增/编辑 -->
			<el-dialog :title="popUp === 'add' ? '新增退货原因' : '编辑退货原因'" :visible.sync="upLoad" width="40%" center>
				<!-- form表单 -->
			...
			<!-- 查询 -->
			<div class="time">
				<el-button type="primary" @click.stop="addUser">新增退货原因</el-button>
	...
	export default{
		...
		data() {
			return {
				...
				serachForm: {keyword: ''},
				// 搜索
				formLabel: [{model: 'keyword',label: '原因类型',type: ''}],
				// 表格
				tabLabel: [
					{label: '原因类型',prop: 'name'},
					{label: '排序',prop: 'sort'},
		...
		methods: {
			async init() {
				...
				const res = await this.axios.get('/rest/ReturnSet',{
					params:{
			...
			del(row){//删除
				this.$confirm('您确定删除该条数据吗？','提示',{
					...
					const res = await this.axios.delete(`/rest/ReturnSet/${row._id}`)
			...
			async confirm() {//确定
				...
				let res;
				let msg = this.popUp === 'edit' ? '编辑退货原因' : '新增退货原因';
				if (this.popUp === 'edit') {
					res = await this.axios.put(`/rest/ReturnSet/${this.id}`, this.upLoadForm)
					this.upLoad = false
					this.init()
			
				} else {
					res = await this.axios.post('/rest/ReturnSet', this.upLoadForm)
					this.upLoad = false
					this.init()
				}
				this.$notify({
